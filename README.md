...
- Conta OCI Trial ativa, com a região **US East (Chicago)** (`us-chicago-1`) assinada e selecionada no Console. Veja o passo 1 se você ainda não tem uma.
...
3. Quando for pedida a home region, escolha **US East (Chicago)** (`us-chicago-1`). Essa escolha é definitiva: depois de criada, a tenancy não muda de home region.
...
Se a sua tenancy já foi criada em outra home region, assine **US East (Chicago)** em **Region Management** e selecione `us-chicago-1` no seletor de região do Console antes de criar a Stack. O Resource Manager usa a região selecionada na sessão atual.
...
| `region` | Região OCI com OCI Generative AI disponível. Auto-preenchida pelo Resource Manager com a região da sua sessão; para este lab, use Chicago (`us-chicago-1`). |
